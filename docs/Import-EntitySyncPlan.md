...
Import is the boundary where reviewer notes turn into plan state. `ReviewerNotes` are appended to the item's `Reasons`, so the operator's decision context carries through to `-WhatIf` and apply output. Plan items whose `Reasons` contain substrings such as `password`, `secret`, `token`, `credential`, `authorization`, or `bearer` were already redacted to `[credential redacted]` during export, and the redaction is preserved on import. Adapter-specific secrets such as the LCAT bearer token are not stored in plan artifacts, so they cannot leak back into apply.
