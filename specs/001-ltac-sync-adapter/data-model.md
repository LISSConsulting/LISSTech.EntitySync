# Data Model: LTAC Customer Scope Sync

## LTAC Customer Scope

Represents a customer scope in LTAC materialized from an N-central customer or site.

### Fields

- `vendor`: Constant `LTAC`.
- `entity_type`: Constant `Customer`.
- `id`: Existing LTAC customer-scope identifier when read back from LTAC.
- `display_name`: Human-readable customer or site name shown to operators.
- `slug`: Safe LTAC customer-scope slug.
...
- `status`: Active or retired, owned by LTAC sync behavior.
...
- Site-derived LTAC customer scopes reference a parent N-central customer through
  `ncentral_parent_customer_id`.
- Customer-derived LTAC customer scopes have no parent scope requirement.
...
- Previously synced record omitted from approved full sync: active -> retired by LTAC.
- Unsafe or incomplete source record: planned -> review-blocked or failed, no LTAC state change.

## N-central Customer Source

Source customer record used to create or update an LTAC customer scope.
...
- `name`: Display name used for LTAC display name and slug derivation.
...
- Source metadata must not include N-central registration tokens in LTAC payloads.

## N-central Site Source

Source site record used to create or update an LTAC customer scope while preserving parent context.
...
Reviewable artifact that carries proposed LTAC actions and safety evidence.
...
- `target_candidates`: Existing LTAC customer scopes when available.

### Validation Rules

- Discovery and planning do not mutate LTAC.
- Only approved write-capable decisions enter the LTAC batch payload.
- Review, reject, no-update, none, unsafe, duplicate, or incomplete items do not mutate LTAC.
