@{
  ...
  PrivateData          = @{
    PSData = @{
      Tags         = @('lisstech', 'sync', 'netsuite', 'halopsa', 'ncentral', 'billcom', 'sophoscentral', 'agentcontroller', 'ltac', 'matching', 'integration')
      ProjectUri   = 'https://github.com/LISSConsulting/LISSTech.EntitySync'
      ReleaseNotes = 'PowerShell 7 entity sync module with adapters for NetSuite, HaloPSA, N-central, Bill.com, Sophos Central, and AgentController (LTAC). Sophos Central partner and organization tenants are available as Customer entities through OAuth client credentials; partner credentials can also create tenants and update their display names. Plans are explainable and reviewer-friendly via Excel/JSON artifacts, and PowerShell and MCP apply operations perform the same target writes, HaloPSA source writebacks, and AgentController authoritative batches. HaloPSA-to-Bill.com plans reconcile the complete active client value list: additions use POST, reviewed removals use BILL''s separate irreversible DELETE endpoint, and rename cleanup occurs only after the new BILL ID is written back to HaloPSA. HaloPSA, N-central, NetSuite, Bill.com, and Sophos Central honour 429 TooManyRequests with up to six retries (Retry-After delta-seconds or future-date, else Math.Min(300, 15 * 2^attempt)) backed by a 500ms inter-request throttle. Bearer tokens, API tokens, OAuth secrets, registration tokens, and other credentials are redacted from plan artifacts, connection output, and adapter error paths; AgentController accepts the operator JWT either as -Token or as a -SecureToken SecureString. Named connection profiles can be saved locally with current-user Windows DPAPI protection and reused with Connect-EntitySyncProfile; AgentController profiles save a DeviceAssetOps profile reference and mint fresh short-lived tokens instead of persisting bearer tokens.'
    }
  }
}
