...
## Canonical audit-correlation fix round 5 — sparse approved operation subsets

- RED on live PostgreSQL proved the repository rejected a valid operation graph containing only approved plan ordinal `5`: its process-local range check incorrectly compared the authoritative plan ordinal with the operation subset count and failed before persistence.
- Repository validation now requires only the invariants it owns—nonnegative, unique item indexes. The existing migration `020` trigger remains authoritative for matching each operation item index to its referenced plan item ordinal.
- The focused repository integration test inserts a six-item durable plan, persists an operation containing only plan ordinal `5`, reconstructs both operation and item through a new `PostgresSyncOperationRepository`, and exercises the production correlation factory to prove `EntityWriteCorrelation.ItemIndex` remains exactly `5`. A deliberately wrong index `4` passes process validation, reaches PostgreSQL, fails with SQLSTATE `55000`, and leaves no operation behind.
- Durable worker dispatch and outcome reconciliation now share the same correlation reconstruction function, which uses only the persisted operation/run/plan/correlation identities and persisted item index. No new identifier is generated or inferred.
- GREEN on live PostgreSQL: the exact sparse integration test passed 1/1; combined `ControlPlaneMigrationTests`, `DurableOperationTests`, and sparse repository coverage passed 42/42. Application and Runtime Release builds passed with 0 warnings and 0 errors; the Platform.Tests Release build succeeded.
- Security self-review: the change relaxes only an invalid process-local subset assumption while retaining nonnegative/unique validation and the database authority check. Correlation remains fail-closed for missing durable run/correlation IDs, and no payload, credential, token, secret, or raw vendor value is logged or persisted.
