...
- Stable plan IDs derive through `EntitySyncCanonicalDigest` from policy/generation identity, selection bounds, actor/timestamps, active exclusions, ordered planner evidence, and canonical entity hashes. Item IDs and source keys use the same shared primitive. The PostgreSQL insert takes a deterministic plan advisory lock and treats only an exact digest/item-count retry as idempotent.
- Desired payloads contain only policy-allowed fields. Blocked fields and write-request IDs never enter payloads or diffs. JSON objects are recursively key-sorted and explicit nulls are retained.
...
- No second serializer/hash convention was introduced: plan identity, source keys, item IDs, item digests, field hashes, payload hashes, audit hashes, and the sealed manifest all use `EntitySyncCanonicalDigest`.
...
- Focused serialization assertions confirm the sensitive test value and blocked field never occur in the persisted manifest projection.
